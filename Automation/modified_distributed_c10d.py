...
_my_new_slim_group = None
def get_new_working_group():
    global _my_new_slim_group
    timeout = timedelta(minutes=0.0001)  # 8 seconds
    ranks = [0]
    if _my_new_slim_group is None:
        _my_new_slim_group = torch.distributed.new_group(ranks, timeout=timeout)
    return _my_new_slim_group
...
def broadcast(tensor, src, group=None, async_op=False):
    ...
    """
    if is_zero_gpu_on() and not is_faulty_gpu():
        group = get_new_working_group()
